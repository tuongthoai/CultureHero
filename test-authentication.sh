...
# CultureHero Authentication System Test Script
# This script demonstrates all the authentication features for different stakeholders
...
curl -X GET "$BASE_URL/api/v1/users" \
  -u "consultant_admin:password123" | jq .

echo ""
echo "🏢 Client Admin - View company users only:"
curl -X GET "$BASE_URL/api/v1/users" \
  -u "client_admin1:password123" | jq .
